...
describe("HappyThoughts Phase 2", () => {
  ...
  it("GET /ask returns the web ask form", async () => {
    const env = makeEnv();
    const res = await worker.fetch(
      new Request("https://test/ask", { method: "GET" }),
      env,
      {} as any
    );
    expect(res.status).toBe(200);
...
describe("HappyThoughts plan entitlements", () => {
  ...
  it("verified fallback stays clearly caveated when verification providers fail", async () => {
    ...
    try {
      ...
      expect(json.thought).toMatch(/\n- /);
      expect(json.thought.split(/\n+/).length).toBeLessThanOrEqual(5);
  ...
  it("creates a live paypal order when client secret is configured", async () => {
    const env = {
      ...makeEnv(),
    ...
    globalThis.fetch = (async (input: any, init?: RequestInit) => {
      ...
      if (url === "https://paypal.test/v2/checkout/orders") {
        return new Response(JSON.stringify({
          id: "PAYPAL_ORDER_123",
          status: "CREATED",
          links: [{ rel: "approve", href: "https://paypal.example/approve/PAYPAL_ORDER_123" }]
    ...
    try {
      ...
      expect(json.order_id).toBe("PAYPAL_ORDER_123");
      expect(json.approval_url).toBe("https://paypal.example/approve/PAYPAL_ORDER_123");
    } finally {
      globalThis.fetch = originalFetch;
    }
  });
